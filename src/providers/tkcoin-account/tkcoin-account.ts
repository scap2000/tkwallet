...
import { TKCoinProvider } from '../tkcoin/tkcoin';

@Injectable()
export class TKCoinAccountProvider {
  /*
  ...
  public pair(
    ...
    this.checkOtp(pairData, otp => {
      ...
      this.bitPayProvider.postAuth(
            ...
            if (pairingReason) {
              ...
              msg = this.replaceParametersProvider.replace(
                this.translate.instant(
                  'To {{reason}} you must first add your TKCoin account - {{email}}'
                ),
                { reason, email }
              );
            } else {
              let email = pairData.email;
              msg = this.replaceParametersProvider.replace(
                this.translate.instant('Add this TKCoin account ({{email}})?'),
                { email }
              );
            }
        ...
        data => {
          return cb(
            this._setError('TKCoin service BitAuth create token: ERROR ', data)
          );
        }
      );
    });
  ...
  private checkOtp(pairData, cb: (otp?) => any) {
    if (pairData.otp) {
      let msg = this.translate.instant(
        'Enter Two Factor for your TKCoin account'
      );
