...
@Injectable()
export class TKCoinIdProvider {
  private NETWORK: string;
  private TKCOIN_API_URL: string;
        ...
        async () => {
          ...
          try {
            ...
            if (user) {
              ...
              await Promise.all([
                this.persistenceProvider.setTKCoinIdPairingToken(
                  network,
                  token.data
                ),
  ...
  public async apiCall(
    ...
    let token =
      userShopperToken ||
      (await this.persistenceProvider.getTKCoinIdPairingToken(
        Network[this.NETWORK]
      ));
    const json = {
      method,
  ...
  public async refreshUserInfo() {
    ...
    const network = Network[this.getEnvironment().network];
    await this.persistenceProvider.setTKCoinIdUserInfo(network, userInfo);
  }

  public async unlockInvoice(invoiceId: string): Promise<string> {
    const isPaired = !!(await this.persistenceProvider.getTKCoinIdPairingToken(
      Network[this.NETWORK]
    ));
  ...
  public async disconnectTKCoinID(successCallback, errorCallback) {
    const network = Network[this.getEnvironment().network];

    // @ts-ignore
    const user: any = await this.persistenceProvider.getTKCoinIdUserInfo(
      network
    );
