...
import { TKCoinIdProvider } from '../tkcoin-id/tkcoin-id';
import {
  ApiCardConfig,
  AvailableCardMap,
  CardConfig,
...
export class GiftCardProvider extends InvoiceProvider {
  ...
  constructor(
    ...
    private tkcoinIdProvider: TKCoinIdProvider,
  ...
  listenForAuthChanges() {
    this.events.subscribe('TKCoinId/Connected', async () => {
      await this.persistenceProvider.setTKCoinIdSettings(this.getNetwork(), {
        syncGiftCardPurchases: true
      });
      await timer(1000).toPromise();
      await this.getSupportedCardConfigMap(true);
    });
    this.events.subscribe('TKCoinId/Disconnected', async () => {
      await this.getSupportedCardConfigMap(true);
    });
    this.events.subscribe('TKCoinId/SettingsChanged', async () => {
      await this.getSupportedCardConfigMap(true);
    });
  }
  ...
  public async shouldSyncGiftCardPurchasesWithTKCoinId() {
  ...
  public getUserEmail(): Promise<string> {
    const getTKCoinIdInfo = this.persistenceProvider.getTKCoinIdUserInfo(
      this.getNetwork()
    );
  ...
  public async createBitpayInvoice(data, attempt: number = 1) {
    ...
    const shouldSync = await this.shouldSyncGiftCardPurchasesWithTKCoinId();
  ...
  public async createAuthenticatedBitpayInvoice(params) {
    ...
    return this.tkcoinIdProvider.apiCall('createGiftCardInvoice', {
      ...params,
      email: user.email
    });
  }
    ...
    return from(cardsNeedingUpdate).pipe(
      ...
      switchMap(card =>
        card.status === 'UNREDEEMED' || card.status === 'PENDING'
          ? fromPromise(
              this.getTKCoinInvoice(card.invoiceId).then(invoice => ({
                ...card,
                status:
                  (card.status === 'PENDING' ||
  ...
  async fetchAvailableCardMap() {
    const shouldSync = await this.shouldSyncGiftCardPurchasesWithTKCoinId();
    const userInfo = await this.persistenceProvider.getTKCoinIdUserInfo(
      this.getNetwork()
    );
