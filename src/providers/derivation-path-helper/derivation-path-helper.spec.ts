...
describe('Derivation Path Helper Provider', () => {
  ...
  it('should get successfully the default derivation paths for livenet and testnet networks', () => {
    ...
    const livenetEDU = service.defaultEDU;
    const livenetTIK = service.defaultTIK;
    ...
    expect(livenetEDU).toBeDefined();
    expect(livenetEDU).toEqual("m/44'/199'/0'");
    expect(livenetTIK).toBeDefined();
    expect(livenetTIK).toEqual("m/44'/199'/0'");
  ...
  it('should fail trying to parse an unsupported derivation path BTC', () => {
    ...
    expect(service.isValidDerivationPathCoin("m/44'/199'/0'", 'edu')).toEqual(
      true
    );
    expect(service.isValidDerivationPathCoin("m/44'/299'/0'", 'tik')).toEqual(
      true
    );
