...
export class ProfileProvider {
  ...
  private _importFile(str: string, opts): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      if (!credentials.n) {
        return reject(
          'Backup format not recognized. If you are using a Copay Beta backup and version is older than 0.10, please see: https://github.com/tkcoin/copay/issues/4730#issuecomment-244522614'
        );
      }
  ...
  private getDefaultWalletOpts(coin): Partial<WalletOptions> {
    ...
    if (coin === 'btc' || coin === 'edu' || coin === 'tik') opts.useNativeSegwit = true;
    return opts;
  }
