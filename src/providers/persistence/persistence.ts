...
export class PersistenceProvider {
  ...
  setBitpayAccount(
    ...
    return this.getBitpayAccounts(network).then(allAccounts => {
      ...
      this.logger.info(
        'Storing TKCoin accounts with new account:' + data.email
      );
  ...
  removeAllTKCoinAccounts(network: string) {
    return this.storage.set(Keys.BITPAY_ACCOUNTS_V2(network), {});
  }
  ...
  removeTKCoinIdSettings(network: Network) {
