...
describe('Provider: Incoming Data Provider', () => {
  ...
  class AppProviderMock {
    public info = {};
    constructor() {
      this.info = { name: 'tkcoin', _enabledExtensions: { debitcard: true } };
    }
  }
  ...
  describe('Function: SCANNER Redir', () => {
    ...
    it('Should handle TKCoin Invoices with different coins', () => {
      let data = [
        'bitcoin:?r=https://tkcoin.org/i/CtcM753gnZ4Wpr5pmXU6i9',
        'bitcoincash:?r=https://tkcoin.org/i/Rtz1RwWA7kdRRU3Wyo4YDY',
        'ethereum:?r=https://test.tkcoin.org/i/VPDDwaG7eaGvFtbyDBq8NR'
      ];
      data.forEach(element => {
        expect(
          incomingDataProvider.redir(element, { activePage: 'ScanPage' })
    ...
    it('Should parse valid TKCoin Invoice Url if selectedtransactionCurrency exists with different coins', fakeAsync(() => {
      const data = [
        {
          expires: '2019-11-05T16:29:31.754Z',
          memo:
            'Payment request for TKCoin invoice FQLHDgV8YWoy4vT8n4pKQe for merchant Johnco',
          payProUrl: 'https://test.tkcoin.org/i/FQLHDgV8YWoy4vT8n4pKQe',
          paymentOptions: [
            {
              chain: 'BTC',
              currency: 'BTC',
      ...
      data.forEach(element => {
        ...
        expect(loggerSpy).toHaveBeenCalledWith(
          'Incoming-data: Handling tkcoin invoice'
        );
        tick();
    ...
    it('Should parse valid TKCoin Invoice Url if !selectedtransactionCurrency', fakeAsync(() => {
      const data = [
        {
          expires: '2019-11-05T16:29:31.754Z',
          memo:
    ...
    it('Should handle Bitcoin cash Copay/TKCoin format and CashAddr format plain Address', () => {
      let data = [
    ...
    it('Should handle Bitcoin cash Copay/TKCoin format and CashAddr format URI', () => {
      let data = [
    ...
    it('Should handle Bitcoin cash Copay/TKCoin format and CashAddr format URI with amount', () => {
      let data = [
    ...
    it('Should handle any type of address/coin using TKCoin URI', fakeAsync(() => {
      ...
      const data = [
        'tkcoin:mrNYDWy8ZgmgXVKDb4MM71LmfZWBwGztUK?coin=btc&amount=0.0002&message=asd',
        'tkcoin:1HZJoc4ZKMvyAYcYCU1vbmwm3KzZq34EmU?coin=btc&amount=0.0002&message=asd',
        'tkcoin:0xDF5C0dd7656bB976aD7285a3Fb80C0F6B9604576?coin=eth&amount=0.0002&message=asd',
        'tkcoin:bchtest:qp2gujqu2dsp6zs4kp0pevm2yl8ydx723q2kvfn7tc?coin=bch&amount=0.0002&message=asd',
        'tkcoin:bitcoincash:qpcc9qe5ja73k7ekkqrnjfp9tya0r3d5tvpm2yfa0d?coin=bch&amount=0.0002&message=asd'
      ];
      data.forEach(element => {
        expect(
          incomingDataProvider.redir(element, { activePage: 'ScanPage' })
        ).toBe(true);
        expect(loggerSpy).toHaveBeenCalledWith('Incoming-data: TKCoin URI');
        const address = extractAddress(element);
        let params: URLSearchParams = new URLSearchParams(
          element.replace(`tkcoin:${address}`, '')
        );
    ...
    it('Should not handle TKCoin URI if address or coin is missing (ScanPage)', fakeAsync(() => {
      const data = [
        'tkcoin:?coin=btc&amount=0.0002&message=asd',
        'tkcoin:1HZJoc4ZKMvyAYcYCU1vbmwm3KzZq34EmU?amount=0.0002&message=asd',
        'tkcoin:0xDF5C0dd7656bB976aD7285a3Fb80C0F6B9604576?amount=0.0002&message=asd',
        'tkcoin:bchtest:qp2gujqu2dsp6zs4kp0pevm2yl8ydx723q2kvfn7tc?amount=0.0002&message=asd',
        'tkcoin:?coin=bch&amount=0.0002&message=asd'
      ];
      data.forEach(element => {
        expect(
          incomingDataProvider.redir(element, { activePage: 'ScanPage' })
    ...
    it('Should not handle TKCoin URI if address or coin is missing (SendPage)', fakeAsync(() => {
      const data = [
        'tkcoin:?coin=btc&amount=0.0002&message=asd',
        'tkcoin:1HZJoc4ZKMvyAYcYCU1vbmwm3KzZq34EmU?amount=0.0002&message=asd',
        'tkcoin:?coin=eth&amount=0.0002&message=asd',
        'tkcoin:bchtest:qp2gujqu2dsp6zs4kp0pevm2yl8ydx723q2kvfn7tc?amount=0.0002&message=asd',
        'tkcoin:?coin=bch&amount=0.0002&message=asd'
      ];
      data.forEach(element => {
    ...
    it('Should handle Coinbase URI', () => {
      let data = ['tkcoin://coinbase', 'copay://coinbase'];
      data.forEach(element => {
    ...
    xit('Should handle BitPay Card URI', () => {
      let data = 'tkcoin://tkcoin.org?secret=xxxxx&email=xxx@xx.com';
    ...
    it('Should handle TKCoin redir link', () => {
      let data = ['tkcoin://landing/card'];
      data.forEach(link => {
        expect(incomingDataProvider.redir(link)).toBe(true);
        expect(loggerSpy).toHaveBeenCalledWith(
          'Incoming-data (redirect): TKCoin Redir'
        );
        const redir = link.replace('tkcoin://landing/', '');
        switch (redir) {
          default:
          case 'card':
            let nextView = {
  ...
  describe('Function: parseData', () => {
    ...
    it('Should return the correct type for each kind of data', () => {
      const dataArray = [
        {
          data: 'https://tkcoin.org/i/5GREtmntcTvB9aejVDhVdm',
          expectedType: 'InvoiceUri'
        },
        {
          data: 'https://test.tkcoin.org/i/VPDDwaG7eaGvFtbyDBq8NR',
          expectedType: 'InvoiceUri'
        },
        {
          data: 'bitcoin:?r=https://tkcoin.org/i/CtcM753gnZ4Wpr5pmXU6i9',
          expectedType: 'PayPro'
        },
        {
          data: 'bitcoincash:?r=https://tkcoin.org/i/Rtz1RwWA7kdRRU3Wyo4YDY',
          expectedType: 'PayPro'
        },
        {
          data: 'ethereum:?r=https://test.tkcoin.org/i/VPDDwaG7eaGvFtbyDBq8NR',
          expectedType: 'PayPro'
        },
        {
        ...
        {
          data:
            'tkcoin:mrNYDWy8ZgmgXVKDb4MM71LmfZWBwGztUK?coin=btc&amount=0.0002&message=message',
          expectedType: 'TKCoinUri'
        },
        {
          data:
            'tkcoin:1HZJoc4ZKMvyAYcYCU1vbmwm3KzZq34EmU?coin=btc&amount=0.0002&message=asd',
        ...
        {
          data:
            'tkcoin:0xDF5C0dd7656bB976aD7285a3Fb80C0F6B9604576?coin=eth&amount=1543000000000000000?gasPrice=0000400000000000000',
          expectedType: 'TKCoinUri'
        },
        {
          data:
        ...
        {
          data: 'http://tkcoin.org/',
          expectedType: 'PlainUrl'
        },
        {
          data: 'https://tkcoin.org',
          expectedType: 'PlainUrl'
        },
        {
        ...
        {
          data: 'tkcoin://coinbase',
          expectedType: 'Coinbase'
        },
        {
          data: 'tkcoin://tkcoin.org?secret=xxxxx&email=xxx@xx.com',
          expectedType: 'TKCoinCard'
        },
        {
          data:
            'copay:RTpopkn5KBnkxuT7x4ummDKx3Lu1LvbntddBC4ssDgaqP7DkojT8ccxaFQEXY4f3huFyMewhHZLbtc',
