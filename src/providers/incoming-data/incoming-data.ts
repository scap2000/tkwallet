...
export class IncomingDataProvider {
  ...
  private isValidEducoinUri(data: string): boolean {
    data = this.sanitizeUri(data);
    return !!this.bwcProvider.getBitcoreEdu().URI.isValid(data);
  }

  private isValidTKCoinUri(data: string): boolean {
    data = this.sanitizeUri(data);
  ...
  private isValidEducoinAddress(data: string): boolean {
    return !!(
      this.bwcProvider.getBitcoreEdu().Address.isValid(data, 'livenet') ||
      this.bwcProvider.getBitcoreEdu().Address.isValid(data, 'testnet')
    );
  }

  private isValidTKCoinAddress(data: string): boolean {
    return !!(
      this.bwcProvider.getBitcoreTik().Address.isValid(data, 'livenet') ||
  ...
  private handleEducoinUri(data: string, redirParams?: RedirParams): void {
  ...
  private handleTKCoinUri(data: string, redirParams?: RedirParams): void {
  ...
  private handlePlainEducoinAddress(
    data: string,
    redirParams?: RedirParams
  ): void {
  ...
  private handlePlainTKCoinAddress(
    data: string,
    redirParams?: RedirParams
  ): void {
  ...
  private async handleUnlock(data) {
    try {
      ...
      const result = await this.tkcoinIdProvider.unlockInvoice(invoiceId);
