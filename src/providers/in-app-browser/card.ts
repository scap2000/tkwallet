...
export class IABCardProvider {
  ...
  async graphRequest(request): Promise<any> {
    try {
      ...
      const { priv, pub } = await this.tkcoinIdProvider.getAppIdentity();
  ...
  async pairing(event) {
    ...
    await this.persistenceProvider.removeAllTKCoinAccounts(
      Network[this.NETWORK]
    );

    // generates pairing token and also fetches user basic info and caches both
    this.tkcoinIdProvider.generatePairingToken(
      params,
      async (user: User) => {
        if (user) {
            ...
            const infoSheet = this.actionSheetProvider.createInfoSheet(
              'in-app-notification',
              {
                ...
                body: this.translate.instant(
                  'TKCoin ID successfully connected.'
                )
              }
            );
