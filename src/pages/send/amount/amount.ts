...
import { TKCoinCardTopUpPage } from '../../integrations/tkcoin-card/tkcoin-card-topup/tkcoin-card-topup';
...
export class AmountPage {
  ...
  private checkAmountForBitpaycard(amount: number): void {
    ...
    const isTopUp =
      this.navParams.data.nextPage === 'TKCoinCardTopUpPage' ? true : false;
    if (isTopUp && amount < 1) {
      this.allowSend = false;
    }
  }
