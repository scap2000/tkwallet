...
import { IABCardProvider, TKCoinIdProvider } from '../../providers';
...
import { TKCoinSettingsPage } from '../integrations/tkcoin-card/tkcoin-settings/tkcoin-settings';
...
import { TKCoinIdPage } from './tkcoin-id/tkcoin-id';
...
export class SettingsPage {
  ...
  public tkcoinCardItems = [];
  public showTKCoinCard: boolean = false;
  ...
  public tkcoinIdPairingEnabled: boolean;
  ...
  public openTKCoinIdPage(): void {
    if (this.bitPayIdUserInfo) {
      this.navCtrl.push(TKCoinIdPage, this.bitPayIdUserInfo);
    } else {
  ...
  public openSettingIntegration(name: string): void {
    switch (name) {
      case 'coinbase':
        ...
        break;
      case 'debitcard':
        this.navCtrl.push(TKCoinSettingsPage);
        break;
      case 'buycrypto':
  ...
  public openSupportEncryptPassword(): void {
    const url =
      'https://support.tkcoin.org/hc/en-us/articles/360000244506-What-Does-a-Spending-Password-Do-';
    const optIn = true;
    const title = null;
